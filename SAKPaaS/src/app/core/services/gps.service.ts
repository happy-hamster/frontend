...
export class GpsService {
  ...
  private static HOME_LOCATION: GpsCoordinates = {
    longitude: 10.018343,
    latitude: 51.133481,
    fromDevice: false,
  };

  private coordinates = new BehaviorSubject<GpsCoordinates>(
    GpsService.HOME_LOCATION
  );
  ...
  constructor(
    ...
    if (
      !this.cookieService.isCookieAlreadySet(GpsService.PERMISSION_COOKIE_NAME)
    ) {
      this.askForPermission().then((granted) => {
        if (granted) {
          this.cookieService.allowCookies();
          this.cookieService.setCookie(
            GpsService.PERMISSION_COOKIE_NAME,
            "true"
          );
